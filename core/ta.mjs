...
function normalize(data, marg) {
    marg = (!marg) ? 0 : marg;
    var max = Math.max.apply(null, data.slice()) * (1 + marg), min = Math.min.apply(null, data.slice()) * (1 - marg), norm = [];
    for (var i = 0; i < data.length; i++) norm.push(1 - (max - data[i]) / (max - min));
    return norm;
}
function denormalize(data, norm, marg) {
    marg = (!marg) ? 0 : marg;
    var max = Math.max.apply(null, data.slice()) * (1 + marg), min = Math.min.apply(null, data.slice()) * (1 - marg), dnorm = [];
...
function ssd(data, len) {
    var length = (!len) ? data.length : len, sd = [];
    for (var i = length; i <= data.length; i++) {
        var tmp = data.slice(i - length, i), mean = sma(tmp, length), sum = 0;
...
function ar(data, len) {
    var out = [];
    for (var i = len; i < data.length; i++) {
        var exp = er(data.slice(i - len, i));
        out.push(data[i] - exp);
    }
    return out;
}
...
function avgwin(data) {
    ...
    var avg = sma(wins, wins.length);
    return avg[0];
}
function avgloss(data) {
    var loss = [];
    for (var i = 0; i < data.length; i++) if (data[i] < 0) loss.push(data[i]);
    var avg = sma(loss, loss.length);
    return avg[0];
}
function zscore(data, len) {
...
function wrsi(data, len) {
    ...
    d = wsma(d, length), u = wsma(u, length);
    for (let i in d) arrsi.push(100 - 100 / (1 + (u[i] / d[i])));
    return arrsi;
}
function tsi(data, llen, slen, sig) {
    ...
    var sma1 = ema(mom, long);
    var sma2 = ema(abs, long);
    var ema1 = ema(sma1, short);
    var ema2 = ema(sma2, short);
    for (var i = 0; i < ema1.length; i++) {
        ts.push(ema1[i] / ema2[i]);
    }
    var tma = ema(ts, signal);
...
function ao(data, len1, len2) {
    ...
    for (var i = length2; i <= data.length; i++) {
        ...
        var f = sma(pl, length1),
            s = sma(pl, length2);
        a.push(f[f.length - 1] - s[s.length - 1]);
    }
    return a;
}
function ac(data, len1, len2) {
...
function fib(start, end) { return [start, (end - start) * .236 + start, (end - start) * .382 + start, (end - start) * .5 + start, (end - start) * .618 + start, (end - start) * .786 + start, end, (end - start) * 1.618 + start, (end - start) * 2.618 + start, (end - start) * 3.618 + start, (end - start) * 4.236 + start] }
function pr(data, len) {
...
function ichimoku(data, len1, len2, len3, len4) {
...
function sma(data, len) {
    ...
    for (var i = length, avg = 0; i <= data.length; i++, avg = 0) {
        ...
        for (let q in pl) avg += pl[q];
        sma.push(avg / length);
    }
    return sma;
}
function smma(data, len) {
...
function wma(data, len) {
    ...
    for (var i = length; i <= data.length; i++) {
        var pl = data.slice(i - length, i), average = 0;
        for (let q in pl) average += pl[q] * (Number(q) + 1) / weight;
        wma.push(average);
    }
    return wma;
}
function wsma(data, len) {
...
function hull(data, len) {
    var length = (!len) ? 14 : len, pl = [], hma = [], ewma = wma(data, length), sqn = Math.round(Math.sqrt(length)),
...
function kama(data, len1, len2, len3) {
    len1 = (!len1) ? 10 : len1, len2 = (!len2) ? 2 : len2, len3 = (!len3) ? 30 : len3;
    var ka = sma(data, len1), ka = [ka[ka.length - 1]];
...
function sim(d, length, sims, perc) {
    ...
    for (var i = 0; i < sims; i++) {
        ...
        for (var x = 0; x < length; x++) {
            ...
            for (var y = 1; y < projected.length; y++) {
                var df = dif(projected[y], projected[y - 1]);
                change.push(df);
            }
            var mean = sma(change, change.length),
                std = std(change), random = normsinv(Math.random());
            projected.push(projected[projected.length - 1] * Math.exp(mean[0] - (std * std) / 2 + std * random));
        }
        sd.push(projected);
    }
    return (!perc) ? sd : percentile(sd, perc);
}
...
function cor(data1, data2) {
    var d1avg = sma(data1, data1.length),
        d2avg = sma(data2, data2.length),
        sumavg = 0, sx = 0, sy = 0;
...
function dif(n, o) { return (n - o) / o; }
function log(d) { return d.map(x => Math.log(x)); }
function exp(d) { return d.map(x => Math.exp(x)); }
function drawdown(d) {
    var max = d[0], min = d[0], big = 0;
    for (var y = 1; y < d.length; y++) {
        if (d[y] > max) {
            if (min != 0) {
                var diff = dif(min, max);
...
function aroon_osc(data, len) {
    var length = (!len) ? 25 : len, aroon = [],
        u = aroon.up(data, length),
        d = aroon.down(data, length);
...
function cop(data, len1, len2, len3) {
    ...
    for (var i = max + len3; i < data.length; i++) {
        ...
        r1 = roc(r1, len1), r2 = roc(r2, len2), r1.splice(0, r1.length - r2.length), r2.splice(0, r2.length - r1.length);
...
function kst(data, r1, r2, r3, r4, s1, s2, s3, s4, sig) {
...
function envelope(data, len, p) {
    len = (!len) ? 10 : len; p = (!p) ? 0.005 : p;
    var enve = [];
    for (var i = len; i < data.length; i++) {
        var sm = sma(data.slice(i - len, i), len);
        enve.push([sm[0] * (1 + p), sm[0], sm[0] * (1 - p)]);
    }
    return enve;
}
function chaikin_osc(data, ema1, ema2) {
    ...
    var ef = ema(adl, ema1), es = ema(adl, ema2);
...
function alligator(data, jl, tl, ll, js, ts, ls) {
...
function gator(data, jl, tl, ll, js, ts, ls) {
    jl = (!jl) ? 13 : jl; tl = (!tl) ? 8 : tl; ll = (!ll) ? 5 : ll;
    js = (!js) ? 8 : js; ts = (!ts) ? 5 : ts; ls = (!ls) ? 3 : ls;
    var ret = [],
        jaw = smma(data, jl),
...
function halftrend(data, atrlen, amplitude, deviation) {
    ...
    for (let i = atrlen; i <= data.length; i++) {
        ...
        let dev = deviation * atr2,
            ...
            highma = sma(pl.slice(pl.length - amplitude, pl.length).map(x => x[0]), amplitude),
            lowma = sma(pl.slice(pl.length - amplitude, pl.length).map(x => x[2]), amplitude);
